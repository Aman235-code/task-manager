  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 flex gap-2 opacity-0 transition group-hover:opacity-100">
        <button className="rounded-xl bg-white/70 p-2 text-indigo-500 shadow-sm backdrop-blur hover:bg-indigo-100 hover:text-indigo-600">
          <Pencil size={16} />
        </button>
        <button className="rounded-xl bg-white/70 p-2 text-rose-500 shadow-sm backdrop-blur hover:bg-rose-100 hover:text-rose-600">
          <Trash2 size={16} />
        </button>
      </div>
...
const Badge = ({ text, danger }: { text: string; danger?: boolean }) => {
  ...
  if (danger) {
    return (
      <span className={`${base} bg-gradient-to-r from-red-500 to-rose-500 text-white`}>
        {text}
      </span>
    );
  }
